...
class PDF extends FPDF {
    ...
    function Body() {
        ...
        $sql = "SELECT * FROM bo_Commandes co
        INNER JOIN bo_factures f ON co.Id_commande = f.Id_commande 
        INNER JOIN bo_client cl ON co.id_client = cl.IDCLIENTS 
        WHERE co.Id_commande = ".$_GET['id']."";
        $fact = $DB->query($sql);
        foreach ($fact as $facts) :
            ...
            $date = explode("-", $facts->Date_factures);
            ...
            $this->SetFont('Arial', 'B', 10);
            $this->Cell(50,0,$facts->num_facture,0,0);
    ...
    function Table_Body() {
        ...
        $sql = "SELECT * FROM bo_Commandes c, bo_factures f, bo_ligne_factures l, bo_Articles a WHERE c.Id_commande = f.Id_commande AND f.Id_facture = l.Id_facture AND l.id_article = a.IDARTICLE AND c.Id_commande = ".$_GET['id'];
        $fact = $bdd->query($sql);
        $this->SetFont('Arial', 'B', 10);
        while ($facts = $fact->fetch()) :
            $this->Cell(20,5,$facts['REFERENCE'],'LR',0,'C');
            $this->SetFont('Arial', 'B', 7);
            $this->Cell(30,5,$facts['LIBARTICLE'],'LR',0,);
            $this->SetFont('Arial', 'B', 10);
            $this->Cell(15,5,$facts['quantite'],'LR',0,'C');
            $this->Cell(30,5,$facts['cond'],'LR',0,'C');
            $this->Cell(25,5,'','LR',0,'C');
            $this->Cell(25,5,'','LR',0,'C');
            $this->Cell(20,5,number_format($facts['prix_unitaire'],0,',',' '),'LR',0,'C');
            $this->Cell(25,5,number_format($facts['prix_unitaire'] * $facts['quantite'],0,',',' '),'LR',0,'C');
            $this->Ln();
        endwhile;
        $this->Cell(190,0,'',1,1);
    }
    ...
    function Foot() {
        ...
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(20,10,number_format($facts['Montant_factures'],0,',',' '));
        $this->SetFont('Arial', '', 10);
        $this->Cell(30,10,'Montant TTC');
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(10,10,number_format($facts['Montant_factures'],0,',',' '));
        $this->Ln();
        $this->Cell(30,0,utf8_decode('Montant réglé'));
        $this->Cell(5,0,'0');
        $this->Ln();
        $this->Cell(0,10,utf8_decode('Arrêté la présente facture à la somme de : '.strtoupper(asLetters($facts['Montant_factures'])).' FRANCS CFA '),0,1);
    }
